...
static int (*libc_mkdir)(const char *pathname, mode_t mode);
...
static int (*libc_clock_gettime)(clockid_t clockid, struct timespec *tp);
...
static int (*libc_lstat64)(const char *restrict pathname, struct stat64 *restrict statbuf);
...
static struct dw_saved_sigaction saved_sigsegv;
static struct dw_saved_sigaction saved_sigbus;
static struct dw_saved_sigaction saved_sigtrap;
...
void dw_init_syscall_stubs()
{
	...
	libc_mkdir = dlsym_check(RTLD_NEXT, "mkdir");
	libc_rmdir = dlsym_check(RTLD_NEXT, "rmdir");
	libc_rename = dlsym_check(RTLD_NEXT, "rename");
	libc_unlink = dlsym_check(RTLD_NEXT, "unlink");
	libc_getcwd = dlsym_check(RTLD_NEXT, "getcwd");
	libc_getrandom = dlsym_check(RTLD_NEXT, "getrandom");
	libc_clock_gettime = dlsym_check(RTLD_NEXT, "clock_gettime");
	...
	libc_lstat64 = dlsym_check(RTLD_NEXT, "lstat64");
	...
	libc_pread64 = dlsym_check(RTLD_NEXT, "pread64");
	libc_pwrite64 = dlsym_check(RTLD_NEXT, "pwrite64");
...
static inline struct dw_saved_sigaction *get_saved_sig_handler(int signum)
{
	switch (signum)
	{
	case SIGSEGV:
...
static inline bool is_libpatch_sigtrap_handler(const struct sigaction *act)
{
	if (act == NULL)
		return false;

	if ((act->sa_flags & SA_SIGINFO) && act->sa_sigaction != NULL)
		return act->sa_sigaction == libpatch_on_trap;

	return (void (*)(int))act->sa_handler == (void (*)(int))libpatch_on_trap;
}

static inline bool save_latest_handler(int signum, const struct sigaction *act)
{
...
int sigaction(int signum, const struct sigaction *act, struct sigaction *oldact)
{
	...
	if (signum == SIGTRAP) {
		/*
		 * we skip saving libpatch's SIGTRAP handler.
		 */
		if (is_libpatch_sigtrap_handler(act)) {
	...
	DW_LOG(WARNING, WRAP, "[%u] Updated the saved %s handler to %p\n", gettid(),
		   signum == SIGSEGV ? "SIGSEGV" : "SIGBUS", user_handler);
...
int __sprintf_chk(char *s, int flag, size_t os, const char *fmt, ...)
{
	...
	va_list arg;
	va_start(arg, fmt);
	const int ret = vsprintf(s, fmt, arg);
	va_end(arg);
	return ret;
}
...
int __vfprintf_chk(FILE *restrict stream, int flag, const char *restrict format, va_list arg)
{
	(void)flag;
	return vfctprintf(fputc_wrapper, (void *) stream, format, arg);
}
...
int access(const char *pathname, int mode)
{
	...
	char *npathname = dw_unprotect((void *)pathname);
	dw_check_access((void *)pathname, libc_strlen(npathname) + 1);
	int ret = libc_access(npathname, mode);
	sout();
	return ret;
}
...
int mkdir(const char *pathname, mode_t mode)
{
	...
	char *npathname = dw_unprotect((void *)pathname);
	dw_check_access((void *)pathname, libc_strlen(npathname) + 1);
	int ret = libc_mkdir(npathname, mode);
	sout();
	return ret;
}
...
{
	sin();
	char *noldpath = dw_unprotect((void *)oldpath);
	char *nnewpath = dw_unprotect((void *)newpath);
	dw_check_access((void *)oldpath, libc_strlen(noldpath) + 1);
	dw_check_access((void *)newpath, libc_strlen(nnewpath) + 1);
	int ret = libc_rename(noldpath, nnewpath);
	sout();
	return ret;
}

int unlink(const char *pathname) { sin(); char *npathname = dw_unprotect((void *)pathname); dw_check_access((void *)pathname, libc_strlen(npathname) + 1); int ret = libc_unlink(npathname); sout(); return ret; }
char *getcwd(char *buf, size_t size)
{
	sin();
	char *nbuf = dw_unprotect((void *)buf);
	dw_check_access((void *)buf, size);
	char *ret = libc_getcwd(nbuf, size);
	sout();
	if (ret == nbuf)
		return buf;
	return ret;
}
int clock_gettime(clockid_t clockid, struct timespec *tp) { sin(); dw_check_access((void *)tp, sizeof(struct timespec)); struct timespec *ntp = (struct timespec *)dw_unprotect((void *)tp); int ret = libc_clock_gettime(clockid, ntp); sout(); return ret; }
ssize_t readlink(const char *pathname, char *buf, size_t bufsiz)
{
	sin();
	char *npathname = dw_unprotect((void *)pathname);
	dw_check_access((void *)pathname, libc_strlen(npathname) + 1);
	dw_check_access((void *)buf, bufsiz);
	char *nbuf = dw_unprotect(buf);
	ssize_t ret = libc_readlink(npathname, nbuf, bufsiz);
	sout();
	return ret;
}
...
int stat64(const char *restrict pathname, struct stat64 *restrict statbuf)
{
	sin();
	char *npathname = dw_unprotect((void *)pathname);
	dw_check_access((void *)pathname, libc_strlen(npathname) + 1);
	dw_check_access((void *)statbuf, sizeof(struct stat64));
	struct stat64 *nstatbuf = (struct stat64 *)dw_unprotect((void *)statbuf);
	int ret = libc_stat64 ? libc_stat64(npathname, nstatbuf)
						  : libc_stat(npathname, (struct stat *)nstatbuf);
	sout();
	return ret;
}
...
int fstat64(int fd, struct stat64 *statbuf)
{
	sin();
	dw_check_access((void *)statbuf, sizeof(struct stat64));
	struct stat64 *nstatbuf = (struct stat64 *)dw_unprotect(statbuf);
	int ret = libc_fstat64 ? libc_fstat64(fd, nstatbuf) : libc_fstat(fd, (struct stat *)nstatbuf);
	sout();
	return ret;
}
...
int lstat64(const char *restrict pathname, struct stat64 *restrict statbuf)
{
	...
	dw_check_access((void *)statbuf, sizeof(struct stat64));
	struct stat64 *nstatbuf = (struct stat64 *)dw_unprotect((void *)statbuf);
	int ret = libc_lstat64 ? libc_lstat64(npathname, nstatbuf) : libc_lstat(npathname, (struct stat *)nstatbuf);
	sout();
	return ret;
}
...
{
	sin();
	dw_check_access(buf, count);
	void *nbuf = dw_unprotect(buf);
	ssize_t ret = libc_pread64 ? libc_pread64(fd, nbuf, count, offset)
							   : libc_pread(fd, nbuf, count, (off_t)offset);
	sout();
	return ret;
}
...
ssize_t pwrite64(int fd, const void *buf, size_t count, off64_t offset)
{
	sin();
	dw_check_access(buf, count);
	const void *nbuf = (const void *)dw_unprotect(buf);
	ssize_t ret = libc_pwrite64 ? libc_pwrite64(fd, nbuf, count, offset)
								: libc_pwrite(fd, nbuf, count, (off_t)offset);
	sout();
	return ret;
}
...
void *__memmove_chk(void *dest, const void *src, size_t len, size_t destlen) { sin(); dw_check_access(dest, destlen); dw_check_access(src, len); void *ret = libc_memmove_chk(dw_unprotect(dest), dw_unprotect(src), len, destlen); sout(); return dw_reprotect(ret, dest); }
void *mempcpy(void *restrict dest, const void *restrict src, size_t n) { sin(); dw_check_access(dest, n); dw_check_access(src, n); void* ret = libc_mempcpy(dw_unprotect(dest), (void *)dw_unprotect(src), n); sout(); return dw_reprotect(ret, dest); }
void *memset(void *s, int c, size_t n) { sin(); dw_check_access(s, n); libc_memset((void *)dw_unprotect(s), c, n); sout(); return s; }
void *__memset_chk(void *s, int c, size_t n, size_t destlen) { sin(); dw_check_access(s, destlen); void *ret = libc_memset_chk(dw_unprotect(s), c, n, destlen); sout(); return dw_reprotect(ret, s); }
...
char *__strcpy_chk(char *dest, const char *src, size_t destlen) { sin(); size_t srclen = libc_strlen(dw_unprotect(src)) + 1; dw_check_access(dest, destlen); dw_check_access(src, srclen); char *ret = libc_strcpy_chk(dw_unprotect(dest), dw_unprotect(src), destlen); sout(); return dw_reprotect(ret, dest); }
...
char *__strcat_chk(char *dest, const char *src, size_t destlen) { sin(); size_t srclen = libc_strlen(dw_unprotect(src)) + 1; dw_check_access(dest, destlen); dw_check_access(src, srclen); char *ret = libc_strcat_chk(dw_unprotect(dest), dw_unprotect(src), destlen); sout(); return dw_reprotect(ret, dest); }
char *__strncat_chk(char *dest, const char *src, size_t n, size_t destlen) { sin(); size_t srclen = libc_strlen(dw_unprotect(src)) + 1; dw_check_access(dest, destlen); dw_check_access(src, srclen < n ? srclen : n); char *ret = libc_strncat_chk(dw_unprotect(dest), dw_unprotect(src), n, destlen); sout(); return dw_reprotect(ret, dest); }
...
char *__strncpy_chk(char *dest, const char *src, size_t n, size_t destlen) { sin(); size_t srclen = libc_strlen(dw_unprotect(src)) + 1; dw_check_access(dest, destlen); dw_check_access(src, srclen < n ? srclen : n); char *ret = libc_strncpy_chk(dw_unprotect(dest), dw_unprotect(src), n, destlen);sout(); return dw_reprotect(ret, dest); }
size_t strlcpy(char *restrict dest, const char *restrict src, size_t size) { sin(); char *ndest = dw_unprotect((void *)dest); char *nsrc = dw_unprotect((void *)src); size_t src_len = libc_strlen(nsrc); dw_check_access(dest, size); dw_check_access(src, src_len + 1); size_t to_copy = 0; if (size != 0) { to_copy = src_len >= size ? size - 1 : src_len; if (to_copy > 0) libc_memcpy(ndest, nsrc, to_copy); ndest[to_copy] = '\0'; } sout(); return src_len; }
size_t strlcat(char *restrict dest, const char *restrict src, size_t size) { sin(); char *ndest = dw_unprotect((void *)dest); char *nsrc = dw_unprotect((void *)src); size_t src_len = libc_strlen(nsrc); dw_check_access(dest, size); dw_check_access(src, src_len + 1); char *d = ndest; const char *s = nsrc; size_t n = size; size_t dlen; while (n != 0 && *d != '\0') { d++; n--; } dlen = (size_t)(d - ndest); n = size - dlen; if (n == 0) { sout(); return dlen + src_len; } while (*s != '\0') { if (n != 1) { *d++ = *s; n--; } s++; } *d = '\0'; sout(); return dlen + src_len; }
...
char *dgettext (const char * domainname, const char * msgid) { sin(); char *ndomainname = dw_unprotect((void *)domainname); char *nmsgid = dw_unprotect((void *)msgid); if (ndomainname) dw_check_access((void *)domainname, libc_strlen(ndomainname) + 1); dw_check_access((void *)msgid, libc_strlen(nmsgid) + 1); char *ret = libc_dgettext(ndomainname, nmsgid); sout(); if(ret == nmsgid) return (char *)msgid; else return ret; }
char *dcgettext (const char * domainname, const char * msgid, int category) { sin(); char *ndomainname = dw_unprotect((void *)domainname); char *nmsgid = dw_unprotect((void *)msgid); if (ndomainname) dw_check_access((void *)domainname, libc_strlen(ndomainname) + 1); dw_check_access((void *)msgid, libc_strlen(nmsgid) + 1); char *ret = __dcgettext (ndomainname, nmsgid, category); sout(); if(ret == nmsgid) return (char *)msgid; else return ret; }
...
char *dcngettext(const char *domainname, const char *msgid, const char *msgid_plural, unsigned long int n, int category) { sin(); char *ndomainname = dw_unprotect((void *)domainname); char *nmsgid = dw_unprotect((void *)msgid); char *nmsgid_plural = dw_unprotect((void *)msgid_plural); if (ndomainname) dw_check_access((void *)domainname, libc_strlen(ndomainname) + 1); dw_check_access((void *)msgid, libc_strlen(nmsgid) + 1); dw_check_access((void *)msgid_plural, libc_strlen(nmsgid_plural) + 1); char *ret = libc_dcngettext(ndomainname, nmsgid, nmsgid_plural, n, category); sout(); if(ret == nmsgid) return (char *)msgid; else if(ret == nmsgid_plural) return (char *)msgid_plural; else return ret; }
...
char *textdomain(const char * domainname) { sin(); char *ndomainname = dw_unprotect((void *)domainname); if (ndomainname) dw_check_access((void *)domainname, libc_strlen(ndomainname) + 1); char *ret = libc_textdomain(ndomainname); sout(); return ret; }
