	...
	for (int i = 0; i < x86->op_count; i++) {
		...
		case X86_OP_MEM:
			...
			if (dw_is_protected((void *) base_addr)) {
				...
				// entry->arg_m[arg_m].base_addr =
				// dw_unprotect((void *)base_addr);
				...
				// entry->arg_m[arg_m].index_addr =
				// dw_unprotect((void *)index_addr);
			}
							...
							ucontext_t *uctx)
{
	...
	// to retaint it immediatly, unless it is overwritten by the
	// instruction.
	bool mem_reg_inst = false;
	for (i = 0; i < entry->nb_arg_m; i++) {
		entry->arg_m[i].base_access = entry->arg_m[i].index_access = 0;
	...
	// If all tainted registers are overwritten by the instruction, there is no
	// point in installing a post handler
	if (entry->post_handler) {
		...
		if (i == entry->nb_arg_m) {
			dw_log(WARNING, DISASSEMBLY, "Disabling post-handler at 0x%llx -> %s %s\n",
	 ...
	 */
	if (!mem_reg_inst && entry->post_handler) {
		...
		bool deferred)
{
	...
	if (entry->post_handler && !entry->deferred_post_handler)
		exec_model.type = PATCH_EXEC_MODEL_PROBE_AROUND_STEP;
